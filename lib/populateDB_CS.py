...
for row in c.fetchall():
	...
	while nn < n_pre:
		if nn % 2 == 0: # equal number of HexA/dHexA and HexN
			...
			for k in range(max(0, cp['S'] - (2*cp['N'] + cp['D'] + cp['U']) + (eq*3) - 3),(min(cp['S'],(eq*3))+1)): # go through SO3 possibilities
			...
			for k in range(max(0, cp['S'] - (2*cp['N'] + cp['D'] + cp['U']) + (bg*2 + sm) - 3),(min(cp['S'],(bg*2 + sm))+1)): # go through SO3 possibilities
				# chemical formula
			...
			for k in range(max(0, cp['S'] - (2*cp['N'] + cp['D'] + cp['U']) + (sm*2 + bg) - 3),(min(cp['S'],(sm*2 + bg))+1)): # go through SO3 possibilities
	...
	while nn < n_pre:
		if cp['D'] == 0: # no dHexA in the molecule
				...
				if n_pre % 2 == 0: # equal number of HexA and HexN in precursor
					for x in ['HexA', 'HexN']: # cycle through the monosaccharides to be added to
						...
						for y in ['NR', 'RE']: # cycle through the ends
							for z in xmod['CS'][x][y]: # cycle through the cross-ring cleavage possibilities
								ext = xmod['CS'][x][y][z]
								j = eq + ext['Ac'] # each HexN will be acetylated
								for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U']) + (eq*3) + ext['SO3'] - 3), min(cp['S'], (eq*3) + ext['SO3'])+1): # go through SO3 possibilities
									# chemical formula
				...
				for mas in ['HexA', 'HexN']: # get which monosaccharide there is more of
					...
					for y in ['NR', 'RE']: # cycle through the ends
						for z in xmod['CS'][x][y]: # cycle through the cross-ring cleavage possibilities
							...
							for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U']) + nU + (2*nN) + ext['SO3'] - 3), min(cp['S'], nU + (2*nN) + ext['SO3'])+1): # go through SO3 possibilities
								# chemical formula
			...
			if nn % 2 == 0: # equal number of dHexA/HexA and HexN in fragment
				...
				for z in xmod['CS']['HexA']['NR']: # cycle through the cross-ring cleavage possibilities
					ext = xmod['CS']['HexA']['NR'][z]
					j = eq + ext['Ac'] # each HexN will be acetylated
					for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + (3*eq) + ext['SO3'] - 3), min(cp['S'], (3*eq) + ext['SO3'])+1): # go through SO3 possibilities
						# chemical formula
				...
				# cuts into dHexA
				for z in xmod['CS']['dHexA']['RE']: # cycle through the cross-ring cleavage possibilities
					ext = xmod['CS']['dHexA']['RE'][z]
					j = eq + ext['Ac'] # each HexN will be acetylated
					for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + (3*eq) + ext['SO3'] - 3), min(cp['S'], (3*eq) + ext['SO3'])+1): # go through SO3 possibilities
						# chemical formula
						f = makeDict('formula', D=0, U=eq, X=0, N=eq, A=j, S=k)
					...
					for x in ['HexA', 'HexN']: # cycle through the monosaccharides to be added to
						...
						for y in ['NR', 'RE']: # cycle through the ends
							for z in xmod['CS'][x][y]: # cycle through the cross-ring cleavage possibilities
								ext = xmod['CS'][x][y][z]
								j = eq + ext['Ac'] # each HexN will be acetylated
								for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + (eq*3) + ext['SO3'] - 3), min(cp['S'], (eq*3) + ext['SO3'])+1): # go through SO3 possibilities
									# chemical formula
									f = makeDict('formula', D=0, U=eq, X=0, N=eq, A=j, S=k)
				...
				## add fragments that include dHexA first
				# includes a full dHexA
				for z in xmod['CS']['HexN']['NR']: # cycle through the cross-ring cleavage possibilities
					ext = xmod['CS']['HexN']['NR'][z]
					j = sm + ext['Ac'] # each HexN will be acetylated
					for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + (2*sm + bg) + ext['SO3'] - 3), min(cp['S'], (2*sm + bg) + ext['SO3'])+1): # go through SO3 possibilities
						# chemical formula
				...
				for z in xmod['CS']['dHexA']['RE']: # cycle through the cross-ring cleavage possibilities
					ext = xmod['CS']['dHexA']['RE'][z]
					j = bg # each HexN will be acetylated
					for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + (2*bg + sm) + ext['SO3'] - 3), min(cp['S'], (2*bg + sm) + ext['SO3'])+1): # go through SO3 possibilities
						# chemical formula
						f = makeDict('formula', D=0, U=sm, X=0, N=bg, A=j, S=k)
					...
					for mas in ['HexA', 'HexN']: # get which monosaccharide there is more of
						...
						for y in ['NR', 'RE']: # cycle through the ends
							for z in xmod['CS'][x][y]: # cycle through the cross-ring cleavage possibilities
								ext = xmod['CS'][x][y][z]
								j = nN + ext['Ac'] # each HexN will be acetylated
								for k in range(max(0, cp['S'] - (2*cp['N'] + cp['U'] + cp['D']) + nU + (2*nN) + ext['SO3'] - 3), min(cp['S'], nU + (2*nN) + ext['SO3'])+1): # go through SO3 possibilities
									# chemical formula
									f = makeDict('formula', D=0, U=nU, X=0, N=nN, A=j, S=k)
